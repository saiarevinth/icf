...
if (isset($_POST['upload'])) {
    ...
    if ($image['name'] != '') {
        ...
        $target = "uploads/" . $imageName; // Store in uploads folder
        ...
        if (move_uploaded_file($image['tmp_name'], $target)) {
            $stmt = $conn->prepare("INSERT INTO images (image_name, image_path, position, title, description) VALUES (?, ?, ?, ?, ?)");
...
if (isset($_POST['update'])) {
    ...
    if (!empty($_FILES['edit_image']['name'])) {
        ...
        $getImage = $conn->query("SELECT image_path FROM images WHERE id = $id");
        ...
        if (move_uploaded_file($image['tmp_name'], $target)) {
            $stmt = $conn->prepare("UPDATE images SET image_name=?, image_path=?, position=?, title=?, description=? WHERE id=?");
            $stmt->bind_param("ssissi", $imageName, $target, $position, $title, $description, $id);
        }
    } else {
        // No new image, just update other fields
        $stmt = $conn->prepare("UPDATE images SET position=?, title=?, description=? WHERE id=?");
...
if (isset($_GET['edit'])) {
    $edit_id = $_GET['edit'];
    $result = $conn->query("SELECT * FROM images WHERE id = $edit_id");
...
<body>
    <div class="container">
        <header>
            <h1><i class="fas fa-images"></i> Image Management System</h1>
        ...
        <div class="card">
            ...
            <div class="gallery">
                <?php
                $result = $conn->query("SELECT * FROM images ORDER BY position ASC");
