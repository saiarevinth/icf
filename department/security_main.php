...
if (isset($_POST['upload'])) {
    ...
    if ($image['name'] != '') {
        ...
        $target = "../uploads/security/" . $imageName; // Store in security uploads folder
        
        // Create uploads directory if it doesn't exist
        if (!file_exists('../uploads/security')) {
            mkdir('../uploads/security', 0777, true);
        }

        if (move_uploaded_file($image['tmp_name'], $target)) {
            $stmt = $conn->prepare("INSERT INTO security_images (image_name, image_path, position, title, description) VALUES (?, ?, ?, ?, ?)");
...
if (isset($_POST['update'])) {
    ...
    if (!empty($_FILES['edit_image']['name'])) {
        ...
        $imageName = time() . '_' . basename($image['name']);
        $target = "../uploads/security/" . $imageName;
        
        // Get old image to delete
        $getImage = $conn->query("SELECT image_path FROM security_images WHERE id = $id");
        ...
        if (move_uploaded_file($image['tmp_name'], $target)) {
            $stmt = $conn->prepare("UPDATE security_images SET image_name=?, image_path=?, position=?, title=?, description=? WHERE id=?");
            $stmt->bind_param("ssissi", $imageName, $target, $position, $title, $description, $id);
        }
    } else {
        // No new image, just update other fields
        $stmt = $conn->prepare("UPDATE security_images SET position=?, title=?, description=? WHERE id=?");
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM security_images WHERE id = $id");
...
if (isset($_GET['edit'])) {
    $edit_id = $_GET['edit'];
    $result = $conn->query("SELECT * FROM security_images WHERE id = $edit_id");
...
<head>
    ...
    <title>Security Department - Image Management System</title>
...
<body>
    <div class="container">
        <header>
            <h1><i class="fas fa-images"></i> Security Department - Image Management System</h1>
            <div style="text-align: right; margin-top: 10px;">
                <a href="security.php" class="btn btn-danger btn-sm">
