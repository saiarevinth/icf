...
if (isset($_POST['upload'])) {
    ...
    if ($image['name'] != '') {
        ...
        $target = "../uploads/vigilance/" . $imageName; // Store in vigilance uploads folder
        
        // Create uploads directory if it doesn't exist
        if (!file_exists('../uploads/vigilance')) {
            mkdir('../uploads/vigilance', 0777, true);
        }

        if (move_uploaded_file($image['tmp_name'], $target)) {
            $stmt = $conn->prepare("INSERT INTO vigilance_images (image_name, image_path, position, title, description) VALUES (?, ?, ?, ?, ?)");
...
if (isset($_POST['update'])) {
    ...
    if (!empty($_FILES['edit_image']['name'])) {
        ...
        $imageName = time() . '_' . basename($image['name']);
        $target = "../uploads/vigilance/" . $imageName;
        
        // Get old image to delete
        $getImage = $conn->query("SELECT image_path FROM vigilance_images WHERE id = $id");
        ...
        if (move_uploaded_file($image['tmp_name'], $target)) {
            $stmt = $conn->prepare("UPDATE vigilance_images SET image_name=?, image_path=?, position=?, title=?, description=? WHERE id=?");
            $stmt->bind_param("ssissi", $imageName, $target, $position, $title, $description, $id);
        }
    } else {
        // No new image, just update other fields
        $stmt = $conn->prepare("UPDATE vigilance_images SET position=?, title=?, description=? WHERE id=?");
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM vigilance_images WHERE id = $id");
...
if (isset($_GET['edit'])) {
    $edit_id = $_GET['edit'];
    $result = $conn->query("SELECT * FROM vigilance_images WHERE id = $edit_id");
...
<head>
    ...
    <title>Vigilance Department - Image Management System</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
        /* Copy all styles from electrical_main.php */
        :root {
...
<body>
    <div class="container">
        <header>
            <h1><i class="fas fa-images"></i> Vigilance Department - Image Management System</h1>
            <div style="text-align: right; margin-top: 10px;">
                <a href="vigilance.php" class="btn btn-danger btn-sm">
                    <i class="fas fa-arrow-left"></i> Back to Vigilance
                </a>
