...
if (isset($_POST['upload'])) {
    ...
    if ($image['name'] != '') {
        ...
        $target = "../uploads/accounts/" . $imageName; // Store in accounts uploads folder
        
        // Create uploads directory if it doesn't exist
        if (!file_exists('../uploads/accounts')) {
            mkdir('../uploads/accounts', 0777, true);
        }

        if (move_uploaded_file($image['tmp_name'], $target)) {
            $stmt = $conn->prepare("INSERT INTO accounts_images (image_name, image_path, position, title, description) VALUES (?, ?, ?, ?, ?)");
            if ($stmt) {
...
if (isset($_POST['update'])) {
    ...
    if (!empty($_FILES['edit_image']['name'])) {
        ...
        $imageName = time() . '_' . basename($image['name']);
        $target = "../uploads/accounts/" . $imageName;
        
        // Get old image to delete
        $getImage = $conn->query("SELECT image_path FROM accounts_images WHERE id = $id");
        ...
        if (move_uploaded_file($image['tmp_name'], $target)) {
            $stmt = $conn->prepare("UPDATE accounts_images SET image_name=?, image_path=?, position=?, title=?, description=? WHERE id=?");
            if ($stmt) {
        ...
        $stmt = $conn->prepare("UPDATE accounts_images SET position=?, title=?, description=? WHERE id=?");
        if ($stmt) {
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM accounts_images WHERE id = $id");
...
if (isset($_GET['edit'])) {
    $edit_id = $_GET['edit'];
    $result = $conn->query("SELECT * FROM accounts_images WHERE id = $edit_id");
...
<head>
    ...
    <title>Accounts Department - Image Management System</title>
...
<body>
    <div class="container">
        <header>
            <h1><i class="fas fa-images"></i> Accounts Department - Image Management System</h1>
            <div style="text-align: right; margin-top: 10px;">
                <a href="accounts.php" class="btn btn-danger btn-sm">
