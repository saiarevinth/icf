...
$create_table_sql = "CREATE TABLE IF NOT EXISTS stores_images (
...
if (isset($_POST['upload'])) {
    ...
    if ($image['name'] != '') {
        ...
        $target = "../uploads/stores/" . $imageName; // Store in stores uploads folder
        
        // Create uploads directory if it doesn't exist
        if (!file_exists('../uploads/stores')) {
            mkdir('../uploads/stores', 0777, true);
        }

        if (move_uploaded_file($image['tmp_name'], $target)) {
            $stmt = $conn->prepare("INSERT INTO stores_images (image_name, image_path, position, title, description) VALUES (?, ?, ?, ?, ?)");
            if ($stmt === false) {
            ...
            if ($stmt->execute()) {
                ...
                $error_message = "Error uploading image: " . $stmt->error;
            }
            $stmt->close();
        } else {
...
if (isset($_POST['update'])) {
    ...
    if (!empty($_FILES['edit_image']['name'])) {
        ...
        $imageName = time() . '_' . basename($image['name']);
        $target = "../uploads/stores/" . $imageName;
        
        // Get old image to delete
        $getImage = $conn->query("SELECT image_path FROM stores_images WHERE id = $id");
        ...
        if (move_uploaded_file($image['tmp_name'], $target)) {
            $stmt = $conn->prepare("UPDATE stores_images SET image_name=?, image_path=?, position=?, title=?, description=? WHERE id=?");
            $stmt->bind_param("ssissi", $imageName, $target, $position, $title, $description, $id);
        }
    } else {
        // No new image, just update other fields
        $stmt = $conn->prepare("UPDATE stores_images SET position=?, title=?, description=? WHERE id=?");
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM stores_images WHERE id = $id");
...
if (isset($_GET['edit'])) {
    $edit_id = $_GET['edit'];
    $result = $conn->query("SELECT * FROM stores_images WHERE id = $edit_id");
...
<head>
    ...
    <title>Stores Department - Image Management System</title>
...
<body>
    <div class="container">
        <header>
            <h1><i class="fas fa-images"></i> Stores Department - Image Management System</h1>
            <div style="text-align: right; margin-top: 10px;">
                <a href="stores.php" class="btn btn-danger btn-sm">
