...
if (isset($_POST['upload'])) {
    ...
    if ($image['name'] != '') {
        ...
        $target = "../uploads/sports/" . $imageName; // Store in sports uploads folder
        
        // Create uploads directory if it doesn't exist
        if (!file_exists('../uploads/sports')) {
            mkdir('../uploads/sports', 0777, true);
        }

        if (move_uploaded_file($image['tmp_name'], $target)) {
            $stmt = $conn->prepare("INSERT INTO sports_images (image_name, image_path, position, title, description) VALUES (?, ?, ?, ?, ?)");
...
if (isset($_POST['update'])) {
    ...
    if (!empty($_FILES['edit_image']['name'])) {
        ...
        $imageName = time() . '_' . basename($image['name']);
        $target = "../uploads/sports/" . $imageName;
        
        // Get old image to delete
        $getImage = $conn->query("SELECT image_path FROM sports_images WHERE id = $id");
        ...
        if (move_uploaded_file($image['tmp_name'], $target)) {
            $stmt = $conn->prepare("UPDATE sports_images SET image_name=?, image_path=?, position=?, title=?, description=? WHERE id=?");
            $stmt->bind_param("ssissi", $imageName, $target, $position, $title, $description, $id);
        }
    } else {
        // No new image, just update other fields
        $stmt = $conn->prepare("UPDATE sports_images SET position=?, title=?, description=? WHERE id=?");
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM sports_images WHERE id = $id");
...
if (isset($_GET['edit'])) {
    $edit_id = $_GET['edit'];
    $result = $conn->query("SELECT * FROM sports_images WHERE id = $edit_id");
...
<head>
    ...
    <title>Sports Department - Image Management System</title>
...
<body>
    <div class="container">
        <header>
            <h1><i class="fas fa-images"></i> Sports Department - Image Management System</h1>
            <div style="text-align: right; margin-top: 10px;">
                <a href="sports.php" class="btn btn-danger btn-sm">
