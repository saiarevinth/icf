...
$createTable = "CREATE TABLE IF NOT EXISTS it_images (
...
if (isset($_POST['upload'])) {
    ...
    if ($image['name'] != '') {
        ...
        $target = "../uploads/it/" . $imageName; // Store in it uploads folder
        
        // Create uploads directory if it doesn't exist
        if (!file_exists('../uploads/it')) {
            mkdir('../uploads/it', 0777, true);
        }

        if (move_uploaded_file($image['tmp_name'], $target)) {
            $stmt = $conn->prepare("INSERT INTO it_images (image_name, image_path, position, title, description) VALUES (?, ?, ?, ?, ?)");
            if ($stmt) {
...
if (isset($_POST['update'])) {
    ...
    if (!empty($_FILES['edit_image']['name'])) {
        ...
        $imageName = time() . '_' . basename($image['name']);
        $target = "../uploads/it/" . $imageName;
        
        // Get old image to delete
        $getImage = $conn->query("SELECT image_path FROM it_images WHERE id = $id");
        ...
        if (move_uploaded_file($image['tmp_name'], $target)) {
            $stmt = $conn->prepare("UPDATE it_images SET image_name=?, image_path=?, position=?, title=?, description=? WHERE id=?");
            if ($stmt) {
        ...
        $stmt = $conn->prepare("UPDATE it_images SET position=?, title=?, description=? WHERE id=?");
        if ($stmt) {
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM it_images WHERE id = $id");
...
if (isset($_GET['edit'])) {
    $edit_id = $_GET['edit'];
    $result = $conn->query("SELECT * FROM it_images WHERE id = $edit_id");
...
<head>
    ...
    <title>IT Department - Image Management System</title>
...
<body>
    <div class="container">
        <header>
            <h1><i class="fas fa-images"></i> IT Department - Image Management System</h1>
            <div style="text-align: right; margin-top: 10px;">
                <a href="it.php" class="btn btn-danger btn-sm">
                    <i class="fas fa-arrow-left"></i> Back to IT
                </a>
